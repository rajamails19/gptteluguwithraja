// Regenerate the brave-little-one BASE (1x) audio with gentle word pauses and
// a ~2s pause after every sentence, so young readers can follow line-by-line.
...
// Word pause = short ". " between words. Sentence pause = long "... ... ..." (~2s).
function withPauses(text) {
  return splitSentences(text)
    .map((sentence) => {
      const words = sentence.replace(/[.!?]+["”]?$/u, "").split(/\s+/).filter(Boolean);
      return words.join(". ") + ".";
    })
    .join(" ... ... ... ");
}
...
for (let i = 0; i < chapters.length; i++) {
  ...
  const out = resolve(
    root,
    `src/assets/audio/young/brave-little-one/base/page-${pageNumber}.mp3`,
  );
  ...
  if (!res.ok) throw new Error(`chapter ${pageNumber}: ${res.status} ${await res.text()}`);
  await writeFile(out, Buffer.from(await res.arrayBuffer()));
  console.log(`  ✓ base/page-${pageNumber}.mp3`);
}
console.log("\n✅ Base audio regenerated with word + line pauses.");
